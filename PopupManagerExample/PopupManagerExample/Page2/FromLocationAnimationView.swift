...
struct FromLocationAnimationView: View {
    ...
    var body: some View {
        PopupLink(widthMultiplier: 0.3, heightMultiplier: 0.3, animationSource: .fromPoint) {
            LocationPopupView()
        } label: {
            CellView(infoText: infoText) {
                Text("This entire cell is a 'PopupLink' label. In the initializer for the link, animationSource was set to .fromPoint. This means that the popup will animate from the tap location. The popup accesses the popupTouchLocation Environment Value and displays the coordinates of the tap location realative to the 'PopupManager'.")
            }
        }
