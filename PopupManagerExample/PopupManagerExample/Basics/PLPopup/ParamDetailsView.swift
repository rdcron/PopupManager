...
struct ParamDetailsView: View {
    var body: some View {
        PopupView {
            Text("'widthMultiplier' and 'heightMultiplier' have been covered. 'touchOutsideDismisses' is a Bool which sets the behavior for dismissing by tapping outside the popup. 'animationSource' is an enum that sets whether the popup animates from the center of the label or from the tap location within the label( .fromRect, .fromPoint). 'popup' is a closure defining the popup, and 'label' is a closure defining the label.")
        }
    }
}

struct ParamDetailsView_Previews: PreviewProvider {
